...
def create_excel():
    wb = Workbook()
    ws = wb.active
    ws.title = "Data Café Surabaya Selatan"

    headers = [
        "No", "Nama Café", "Menu & Harga", "Fasilitas",
        "Alamat Lengkap", "Kecamatan", "Kelurahan/Desa",
        "Foto (URL/Nama File)", "Longitude", "Latitude"
    ]
    ...
    for i, cafe in enumerate(cafes):
        ...
        data = [i+1, cafe["nama"], cafe["menu_harga"], cafe["fasilitas"],
                cafe["alamat"], cafe["kecamatan"], cafe["kelurahan"],
                cafe["foto"], cafe["longitude"], cafe["latitude"]]
        for col, val in enumerate(data, 1):
            ...
            cell.font = Font(bold=(col == 2), name="Arial", size=10)

    col_widths = [5, 30, 58, 52, 58, 18, 22, 32, 16, 16]
    for i, w in enumerate(col_widths, 1):
        ws.column_dimensions[get_column_letter(i)].width = w

    ws.row_dimensions[1].height = 30
    for row in range(2, len(cafes) + 2):
    ...
    # Summary sheet
    ws2 = wb.create_sheet("Rekap per Kecamatan")
    sum_headers = ["Kecamatan", "Jumlah Café"]
    ...
    kec_count = Counter(c["kecamatan"] for c in cafes)
    for i, (kec, count) in enumerate(sorted(kec_count.items()), 2):
        ws2.cell(row=i, column=1, value=kec).border = border
        ws2.cell(row=i, column=1).alignment = wrap_align
        ws2.cell(row=i, column=1).font = Font(name="Arial", size=10)
        ws2.cell(row=i, column=2, value=count).border = border
        ws2.cell(row=i, column=2).alignment = center_align
        ws2.cell(row=i, column=2).font = Font(name="Arial", size=10)

    # Total
    total_row = len(kec_count) + 2
    ws2.cell(row=total_row, column=1, value="TOTAL").font = Font(bold=True, name="Arial", size=10)
    ws2.cell(row=total_row, column=1).fill = PatternFill("solid", start_color="BDD7EE")
    ws2.cell(row=total_row, column=1).border = border
    ws2.cell(row=total_row, column=1).alignment = center_align
    ws2.cell(row=total_row, column=2, value=len(cafes)).font = Font(bold=True, name="Arial", size=10)
    ws2.cell(row=total_row, column=2).fill = PatternFill("solid", start_color="BDD7EE")
    ...
    output_file = "cafe_surabaya_selatan.xlsx"
    wb.save(output_file)
    print(f"Done! Created '{output_file}' with {len(cafes)} cafés.")
